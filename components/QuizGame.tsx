...
const QuizGame: React.FC<QuizGameProps> = ({ words, onComplete, onBack }) => {
  ...
  const handleOptionClick = (optionWord: Word) => {
    if (selectedOption !== null) return;

    setSelectedOption(optionWord.id);
    const correct = optionWord.id === currentWord.id;
  ...
  if (quizWords.length < OPTIONS_COUNT) {
    return (
      <div className="text-center p-8 bg-slate-800 rounded-lg shadow-md">
        ...
        <p className="text-slate-300">퀴즈를 풀려면 최소 {OPTIONS_COUNT}개의 단어를 학습해야 해요.</p>
        <button 
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl text-center">
        <p className="text-slate-400 font-semibold mb-2">다음 단어의 뜻은 무엇일까요...</p>
        <h2 className="text-4xl font-extrabold mb-8 text-white">{currentWord?.word}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {options.map(option => {
            ...
            } else if (selectedOption !== null && option.id === currentWord.id) {
