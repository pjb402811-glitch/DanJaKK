...
const FlashcardView: React.FC<FlashcardViewProps> = ({ words, updateWordProgress, onComplete, onBack, sessionTitle }) => {
  ...
  const handleNext = (isCorrect: boolean) => {
    ...
    if (currentIndex < words.length - 1) {
      ...
      const xpGained = (sessionProgress.correct + (isCorrect ? 1 : 0)) * XP_PER_WORD;
      const coinsGained = (sessionProgress.correct + (isCorrect ? 1 : 0)) * COINS_PER_WORD;
  ...
  return (
    <div className="flex flex-col items-center">
        ...
        <div className="w-full max-w-2xl">
          <p className="text-center text-slate-400 font-semibold">{sessionTitle}: {currentIndex + 1} / {words.length}</p>
          <div className="w-full bg-slate-700 rounded-full h-2.5 mt-2 mb-8">
              <div className="bg-blue-500 h-2.5 rounded-full" style={{ width: `${((currentIndex + 1) / words.length) * 100}%` }}></div>
          </div>
        </div>
        ...
        >
          <div className={`relative w-full h-full transform-style-preserve-3d transition-transform duration-500 ${isFlipped ? 'rotate-y-180' : ''}`}>
            ...
            <div className="absolute w-full h-full backface-hidden bg-slate-800 rounded-2xl shadow-xl flex flex-col justify-center items-center p-6 border-4 border-blue-400">
              <h2 className="text-5xl font-extrabold text-white break-all text-center">{currentWord.word}</h2>
            ...
            <div className="absolute w-full h-full backface-hidden bg-slate-800 rounded-2xl shadow-xl flex flex-col justify-center items-center p-6 rotate-y-180 border-4 border-sky-400">
                <p className="text-center break-all">
                    <span className="text-4xl font-bold text-slate-200">{currentWord.meaning}</span>
                </p>
            </div>
          </div>
        </div>
