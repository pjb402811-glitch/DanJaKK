...
interface ConversationQuizGameProps {
...
const ConversationQuizGame: React.FC<ConversationQuizGameProps> = ({ conversations, allConversations, onComplete, onBack }) => {
  const [quizItems] = useState(conversations);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [options, setOptions] = useState<Conversation[]>([]);
  ...
  const generateOptions = useCallback(() => {
    if (!currentItem || allConversations.length < OPTIONS_COUNT) return;

    const otherItems = allConversations.filter(w => w.id !== currentItem.id);
    const shuffled = otherItems.sort(() => 0.5 - Math.random());
    const randomOptions = shuffled.slice(0, OPTIONS_COUNT - 1);
    const allOptions = [...randomOptions, currentItem].sort(() => 0.5 - Math.random());
    setOptions(allOptions);
  }, [currentItem, allConversations, quizItems]);

  useEffect(() => {
    generateOptions();
  }, [currentIndex, generateOptions]);

  const handleOptionClick = (optionItem: Conversation) => {
    if (selectedOption !== null) return;

    setSelectedOption(optionItem.id);
    const correct = optionItem.id === currentItem.id;
  ...
  if (quizItems.length === 0 || allConversations.length < OPTIONS_COUNT) {
    return (
      <div className="text-center p-8 bg-slate-800 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold mb-4">문장 수 부족</h2>
        <p className="text-slate-300">퀴즈를 풀려면 최소 {OPTIONS_COUNT}개의 문장을 학습해야 해요.</p>
        <button 
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      <div className="w-full bg-slate-700 rounded-full h-2.5 mb-4">
          <div className="bg-purple-500 h-2.5 rounded-full transition-all duration-300" style={{ width: `${((currentIndex) / quizItems.length) * 100}%` }}></div>
      </div>
      ...
      <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {options.map(option => {
            ...
            } else if (selectedOption !== null && option.id === currentItem.id) {
...
export default ConversationQuizGame;